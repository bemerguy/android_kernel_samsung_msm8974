...
step3_compile()
{
	...
	# compile source
	if [ -z "$OUTPUT_FOLDER" ]; then
		make -j$NUM_CPUS 2>&1 CONFIG_NO_ERROR_ON_MISMATCH=y |tee ../compile.log
	else
		make -j$NUM_CPUS O=$OUTPUT_FOLDER CC=$CLANG_TC CLANG_TRIPLE=$TC CROSS_COMPILE=$TOOLCHAIN CONFIG_NO_ERROR_ON_MISMATCH=y 2>&1 |tee ../compile.log
	fi
